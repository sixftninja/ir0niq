...
final class RegressionTests: XCTestCase {
    ...
    func testCompleteWorkoutSession() async throws {
        ...
        // 4. Log set 2 (without beginning — unrecorded)
        try await engine.logCurrentSet(reps: 4, weight: 100)
        let context = await engine.sessionContext
        XCTAssertTrue(context?.exercises[0].setContexts[1].isUnrecorded == true,
                      "Set logged without beginCurrentSet should be unrecorded")
    ...
    // MARK: - Edge case 6 regression: unrecorded flag

    func testUnrecordedFlagSetCorrectly() async throws {
        ...
        // Log without beginCurrentSet
        try await engine.logCurrentSet(reps: 10, weight: 0)
        let ctx = await engine.sessionContext
        XCTAssertTrue(ctx?.exercises[0].setContexts[0].isUnrecorded == true)
    }
