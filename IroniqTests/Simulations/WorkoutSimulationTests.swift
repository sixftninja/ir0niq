...
final class WorkoutSimulationTests: XCTestCase {
    ...
    func testSimulation_HappyPathWorkout() async throws {
        ...
        // Do all 3 sets. Sets auto-start; the user only taps Rest, then logs.
        for i in 0..<3 {
    ...
    func testSimulation_SetAutoStartsWithoutManualTap() async throws {
        try await engine.selectTemplate(templateId)
        _ = try await engine.startSession()

        let context = await engine.sessionContext
        if case .inProgress = context?.currentSet?.lifecycleState {
            // Expected: there is no user-facing Start Set action.
        } else {
            XCTFail("Current set should auto-start when the workout opens")
        }
        XCTAssertNotNil(context?.currentSet?.setTimerStart)
    }

    func testSimulation_ForgotToTapRest_ReminderWindowUsesDefaultSetTime() async throws {
        try await engine.selectTemplate(templateId)
        _ = try await engine.startSession()

        let context = await engine.sessionContext
        guard case .inProgress(let startedAt) = context?.currentSet?.lifecycleState else {
            XCTFail("Current set should be in progress")
            return
        }
        let reminderAt = startedAt.addingTimeInterval(120)

        XCTAssertEqual(reminderAt.timeIntervalSince(startedAt), 120)
    }

    func testSimulation_TapRestWithoutManualStartOpensLoggingState() async throws {
    ...
    func testSimulation_StartedRestButForgotToLogAppearsInReview() async throws {
        try await engine.selectTemplate(templateId)
        _ = try await engine.startSession()
        ...
        let unlogged = try await engine.endSession()
        XCTAssertEqual(unlogged.count, 1)
        XCTAssertEqual(unlogged.first?.setOrder, 0)
    }
    ...
    // MARK: - Simulation 6: Auto-started set timing

    func testSimulation_AutoStartedSetIsRecordedWhenLogged() async throws {
        try await engine.selectTemplate(templateId)
        _ = try await engine.startSession()
        ...
        XCTAssertFalse(set?.isUnrecorded == true, "Auto-started set should have a timer start")
