...
struct WorkoutExportSession: Codable, Sendable, Equatable, Identifiable {
  ...
  init(from session: SessionDTO) {
    ...
    exercises = session.exercises.sorted { $0.order < $1.order }.map(
      WorkoutExportExercise.init(from:))
    ...
    for session in sessions {
      ...
      for exercise in session.exercises {
        ...
        for set in exercise.sets {
          let result =
            set.reps.map { "\($0) reps" } ?? set.durationSeconds.map { "\(Int($0)) sec" }
            ?? set.status
...
// MARK: - Template export model (iCloud Drive JSON schema, version 1)

struct TemplateExportModel: Codable, Sendable, Equatable {
  static let currentVersion = 1
  ...
  init(from template: TemplateDTO, exportedAt: Date = Date()) {
    self.version = Self.currentVersion
    self.exportedAt = exportedAt
    self.updatedAt = exportedAt
    self.id = template.id
    self.name = template.name
    self.createdAt = template.createdAt
    self.exercises = template.exercises.sorted { $0.order < $1.order }.map(
      TemplateExerciseExport.init(from:))
  }

  func jsonData() throws -> Data {
    try SessionExportModel.jsonEncoder.encode(self)
  }
}

struct TemplateExerciseExport: Codable, Sendable, Equatable {
  let id: UUID
  let exerciseId: UUID
  let exerciseName: String
  let defaultLoggingType: String
  let order: Int
  let equipmentTypeOverride: String?
  let sets: [TemplateSetExport]

  init(from exercise: TemplateExerciseDTO) {
    id = exercise.id
    exerciseId = exercise.exerciseId
    exerciseName = exercise.exerciseName
    defaultLoggingType = exercise.defaultLoggingType.rawValue
    order = exercise.order
    equipmentTypeOverride = exercise.equipmentTypeOverride?.rawValue
    sets = exercise.sets.sorted { $0.order < $1.order }.map(TemplateSetExport.init(from:))
  }
}

struct TemplateSetExport: Codable, Sendable, Equatable {
  let id: UUID
  let order: Int
  let targetReps: Int?
