...
struct HistoryCalendarView: View {
    ...
    private func dayCell(_ date: Date) -> some View {
        ...
        Button {
            ...
            ZStack {
                Circle()
                    .fill(isSelected ? Color.ironiqOrange : (hasWorkout ? Color.ironiqGreen.opacity(0.3) : Color.clear))
                    .frame(width: 34, height: 34)

                if isToday && !isSelected {
                    Circle()
                        .stroke(Color.ironiqOrange.opacity(0.6), lineWidth: 1.5)
                        .frame(width: 34, height: 34)
                }

                Text("\(calendar.component(.day, from: date))")
                    .font(.system(size: 14, weight: isToday ? .bold : .regular))
                    .foregroundStyle(isSelected ? .black : (hasWorkout ? Color.ironiqGreen : .white.opacity(0.8)))
            }
