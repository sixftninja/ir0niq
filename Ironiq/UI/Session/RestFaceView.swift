...
struct RestFaceView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 12) {
                // Log early button
                IroniqButton("Log Set", style: .primary) {
                    showInput = true
                }
        ...
        .sheet(isPresented: $showInput) {
            InputFaceView(set: set, defaultLoggingType: vm.currentExercise?.defaultLoggingType ?? .reps, onLog: { r, durationSeconds, w in
                Task {
                    await vm.logCurrentSet(reps: r, durationSeconds: durationSeconds, weight: w)
                    showInput = false
                }
            })
