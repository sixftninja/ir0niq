    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if case .inProgress = set.lifecycleState {
                VStack(spacing: 8) {
                    Text("SET TIME")
                        ...
                        .tracking(2)
                    TimerView(value: vm.setElapsed, color: .forgeOrange)
            ...
            if case .pending = set.lifecycleState {
                ForgeButton("Begin Set", style: .primary) {
                ...
                VStack(spacing: 12) {
                    ForgeButton("Rest", style: .secondary) {
...
struct ForgeButton: View {
    ...
    var body: some View {
        Button(action: action) {
            Text(title)
                .font(.headline)
                .foregroundStyle(style == .primary ? .black : Color.forgeOrange)
                .frame(maxWidth: .infinity)
                .padding()
                .background(style == .primary ? Color.forgeOrange : Color.forgeOrange.opacity(0.12))
                .clipShape(RoundedRectangle(cornerRadius: 14))
                .overlay(
                    RoundedRectangle(cornerRadius: 14)
                        .stroke(style == .secondary ? Color.forgeOrange.opacity(0.4) : .clear, lineWidth: 1)
                )
        }
    }
}
