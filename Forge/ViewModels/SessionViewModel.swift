...
final class SessionViewModel {
    ...
    func startAdHocSession() async {
        do { _ = try await engine.startAdHocSession() }
        catch { setAlert(error) }
    }
    ...
    func logCurrentSet(reps: Int?, weight: Double?) async {
        do { try await engine.logCurrentSet(reps: reps, weight: weight) }
        catch { setAlert(error) }
        await refreshContext()
    }
    ...
    func addUnplannedExercise(exerciseId: UUID, exerciseName: String, setCount: Int) async {
        do { try await engine.addUnplannedExercise(exerciseId: exerciseId, exerciseName: exerciseName, setCount: setCount) }
        catch { setAlert(error) }
        await refreshContext()
    }
    ...
    private func tick() async {
        ...
        let now = Date()
        sessionElapsed = now.timeIntervalSince(ctx.sessionStartedAt) - ctx.totalPauseDuration
        ...
        case .resting(_, let target):
            ...
            if let t = target {
                ...
                // Auto-transition when rest ends
                if remaining <= 0 {
                    try? await Task.sleep(for: .milliseconds(200))
                    await self.restEnded()
                }
            }
        default:
