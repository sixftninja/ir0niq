...
enum SetLifecycleState: Equatable, Sendable {
    ...
    case awaitingInput
    case logged(reps: Int?, weight: Double?)
    case notPerformed
}
...
actor SessionEngine {
    ...
    func startSession() async throws -> UUID {
        ...
        return try await buildAndActivateSession(templateId: templateId, exercises: template.exercises)
    }
    ...
    func startAdHocSession() async throws -> UUID {
        ...
        return try await buildAndActivateSession(templateId: nil, exercises: [])
    }
    ...
    func beginCurrentSet() throws {
        ...
        guard case .pending = context.currentExercise!.setContexts[context.currentSetIndex].lifecycleState else {
    ...
    func logCurrentSet(reps: Int?, weight: Double?) async throws {
        guard case .active = state else {
        ...
        set.lifecycleState = .logged(reps: reps, weight: weight)
    ...
    func addUnplannedExercise(exerciseId: UUID, exerciseName: String = "", setCount: Int) async throws {
        ...
        var setContexts: [ActiveSessionContext.ExerciseContext.SetContext] = []
        for i in 0..<setCount {
            let setId = try await sessionRepository.addSet(to: sessionExerciseId, order: i)
            setContexts.append(.init(sessionSetId: setId, order: i))
        }
